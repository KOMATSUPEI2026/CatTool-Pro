...
## 6. 批 5：字級刻度變數化＋整組 +2px（2026-07-17，V50）

使用者長時間使用回饋「畫面字有點小」定案：五級刻度 **10/12/14/16/24 → 12/14/16/18/26**。

- 全 100 處 `font-size` 收斂為五變數：`--fs-xs`（迷你註記）/`--fs-sm`（UI 骨架最大宗）/`--fs-md`（內文）/`--fs-lg`（原文/Modal 標題）/`--fs-xl`（H1）；×`--text-scale`／×`--side-scale` 的 calc 形式一併改掛變數
- **今後規範：字級一律經 `--fs-*` 變數，禁寫字面 px**；刻度掃描基準同步改 12/14/16/18/26
- spacing 決策＝**不全面 +2px**（2~40px 值域平移會比例失真）；行高全站相對值自動吸收，八畫面截圖驗傷後間距零調整
- 驗證：`npm run build`＋八畫面截圖（入稿/專案/工作區/TM 側欄/歷史側欄/句段 Modal/術語庫/快捷鍵 Modal）＋r3~r6/phase0/phase2/phase3 七套全綠

## 7. 批 5 補充：字級雙模式＋歷史紀錄橘（2026-07-17，V51）

- 五級變數加 `data-font-mode` 雙刻度：桌機（預設）＝12/14/16/18/26、筆電＝10/12/14/16/24（27 吋 vs 13.3 吋各有舒適刻度，使用者定案兩組都保留，頂列鈕切換、localStorage `catToolFontMode` 記憶）
- 新 token `--hist-accent`：歷史紀錄專屬柿茶橘（亮 #B4622D／暗 #DD8F53，比照朱紅/teal 亮暗一階慣例），套把手與還原鈕——右側三把手三色可辨
