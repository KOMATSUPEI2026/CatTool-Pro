...
一批一 commit，每批 `npm run build` ＋開 dev server 目視比對明暗兩主題：

| 批次 | 內容 | 風險 |
|---|---|---|
| 批 1 | §3.1 字體變數 ×3＋TmTab.jsx:68 改 class | 極低（純等值替換） |
| 批 2 | §3.3 尺寸耦合 `--side-w`／`--wrap-w` | 極低（4＋2 處） |
| 批 3 | §3.2 圓角刻度補齊 | 低（處數多，逐條替換） |
| 批 4 | §3.4 z-index＋§3.5 陰影＋§3.6 色彩漏網 | 低 |
| 緩辦 | §3.7 全部 | ——（涉及視覺變化，另案討論） |
